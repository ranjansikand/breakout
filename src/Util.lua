...
function GenerateQuadsBricks(atlas)
    bricksAndLock = {}
    bricksAndLock = table.slice(GenerateQuads(atlas, 32, 16), 1, 21)
    table.insert(bricksAndLock, GenerateQuads(atlas, 32, 16)[24])

    return bricksAndLock
end
...
function GenerateQuadsPower(atlas)
    local x = 0 -- I only need one, but may need others later
    local y = 192
